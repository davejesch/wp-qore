...
<div id="icon-options-general" class="icon32"><br></div><h2><?php _e('Login Security', 'loginlockdown') ?></h2>
...
<div style="margin-top:20px;margin-bottom:10px;text-shadow: #fff 0 1px 0;"><?php _e('Max Login Retries', 'loginlockdown') ?></div>

<input type="text" name="ll_max_login_retries" size="8" value="<?php echo esc_attr($loginlockdownAdminOptions['max_login_retries']); ?>">

<div style="margin-top:20px;margin-bottom:10px;text-shadow: #fff 0 1px 0;"><?php _e('Retry Time Period Restriction (minutes)', 'loginlockdown') ?></div>

<input type="text" name="ll_retries_within" size="8" value="<?php echo esc_attr($loginlockdownAdminOptions['retries_within']); ?>">

<div style="margin-top:20px;margin-bottom:10px;text-shadow: #fff 0 1px 0;"><?php _e('Lockout Length (minutes)', 'loginlockdown') ?></div>

<input type="text" name="ll_lockout_length" size="8" value="<?php echo esc_attr($loginlockdownAdminOptions['lockout_length']); ?>">

<div style="margin-top:20px;margin-bottom:10px;v"><?php _e('Lockout Invalid Usernames?', 'loginlockdown') ?></div>
...
<div style="margin-top:20px;margin-bottom:10px;text-shadow: #fff 0 1px 0;"><?php _e('Mask Login Errors?', 'loginlockdown') ?></div>
...
<input class="button button-primary" type="submit" name="update_loginlockdownSettings" value="<?php _e('Update Settings', 'loginlockdown') ?>" /></div>
...
<div style="margin-top:20px;margin-bottom:10px;text-shadow: #fff 0 1px 0;"><?php _e('Currently Locked Out', 'loginlockdown') ?></div>
...
<input type="checkbox" name="releaseme[]" value="<?php echo esc_attr($option['lockdown_ID']); ?>"> <?php echo esc_attr($option['lockdown_IP']); ?> (<?php echo esc_attr($option['minutes_left']); ?> minutes left)

<?php } } ?>

<div class="submit">
<input class="button button-primary" type="submit" name="release_lockdowns" value="<?php _e('Release Selected', 'loginlockdown') ?>" /></div>
</form>
</div>
...
.wrap form{background-color:#F7F7F7;border: 1px solid #e1e1e1;padding-left:10px;width:50%;}
.wrap h2{padding-bottom:25px;}
</style>
...
//adds options page to wp-admin
function loginlockdown_ap() {
	if ( function_exists('add_options_page') ) {

//add_menu_page('Login Security', 'Login Security', 9, basename(__FILE__), 'print_loginlockdownAdminPage');

add_submenu_page( 'wp-qore/functions.php', 'Login Security', 'Login Security', 9, basename(__FILE__), 'print_loginlockdownAdminPage');
