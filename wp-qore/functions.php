...
function wpqorefunc_fp_admin() {
    ...
    add_menu_page('WP Qore Settings', 'WP Qore', 5, __FILE__, 'wpqorefunc_fp_settings');
    add_submenu_page( 'wp-qore/functions.php', 'WP Qore Settings', 'WP Qore Settings', 5, __FILE__, 'wpqorefunc_fp_settings');
}
...
function wpqorefunc_ajax_db_callback() {
    ...
    else echo '<span class="wpqorefunc_red">You have '.$rev_count->cnt.' revision posts in your sql tables. It is not good for performance. But remember that revisions are for keeping old versions of posts If you don\'t need them and want to make your database work faster, then just run this query in your phpmyadmin: <i><pre>DELETE a,b,c FROM '.$wpdb->posts.' a WHERE a.post_type = \'revision\' LEFT JOIN '.$wpdb->term_relationships.' b ON (a.ID = b.object_id) LEFT JOIN '.$wpdb->postmeta.' c ON (a.ID = c.post_id);
 ...
 * @since 1.0.0
 */
add_action( 'init', 'WPQore_updater_init' );

function WPQore_updater_init() {

    /* Load Plugin Updater */
    require_once( trailingslashit( plugin_dir_path( __FILE__ ) ) . 'functions/plugin-updater.php' );
    ...
    'username'  => false,
    'key'       => '',
    'repo_uri'  => 'http://wp-repo.com/',  //required
    'repo_slug' => 'wp-qore',  //required

    );

    /* Load Updater Class */
    new WPQore_Plugin_Updater( $config );
