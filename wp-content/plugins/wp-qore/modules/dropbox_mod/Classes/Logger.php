 ...
 */
class WPQ2DBX_Logger
{
    const LOGFILE = 'wpq2dbx-backup-log.txt';
    ...
    {
        if (!$this->logFile) {
            WPQ2DBX_BackupController::create_dump_dir();

            $path = WPQ2DBX_Factory::get('config')->get_backup_dir() . DIRECTORY_SEPARATOR . self::LOGFILE;

            $files = glob($path . '.*');
            if (isset($files[0])) {
                $this->logFile = $files[0];
            } else {
                $this->logFile = $path . '.' . WPQ2DBX_Factory::secret(self::LOGFILE);
            }
        }
