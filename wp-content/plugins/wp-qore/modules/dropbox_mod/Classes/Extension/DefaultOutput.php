 ...
 */
class WPQ2DBX_Extension_DefaultOutput extends WPQ2DBX_Extension_Base
{
    const MAX_ERRORS = 10;
        ...
        try {
            ...
            if (!in_array(basename($file), $directory_contents) || filemtime($file) > $this->config->get_option('last_backup_time')) {
                ...
                if ($file_size > $this->get_chunked_upload_threashold()) {
                    ...
                    $msg = __("Uploading large file '%s' (%sMB) in chunks", 'wp-qore');
                    if ($processed_file && $processed_file->offset > 0)
                        $msg = __("Resuming upload of large file '%s'", 'wp-qore');
            ...
            WPQ2DBX_Factory::get('logger')->log(sprintf(__("Error uploading '%s' to Dropbox: %s", 'wp-qore'), $file, strip_tags($e->getMessage())));
            $this->error_count++;
        }
    }
