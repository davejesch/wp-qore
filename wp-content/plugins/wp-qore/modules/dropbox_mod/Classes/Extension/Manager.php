 ...
 */
class WPQ2DBX_Extension_Manager
{
    const API_VERSION = 0;
    ...
    {
        $extensions = get_option('wpq2dbx-premium-extensions');
    ...
    {
        return 'admin.php?page=b2dbx-premium';
    }
    ...
    public function get_extensions()
    {
        if (!$this->extensionsCache) {
            ...
            if (is_wp_error($response)) {
                throw new Exception(__('There was an error getting the list of premium extensions', 'wp-qore'));
            }
    ...
    public function install($name)
    {
        ...
        $params = array(
            ...
            'name' => $name,
            'site' => get_site_url(),
            'version' => WPQORE_2_DROPBOX_VERSION,
        );
        ...
        if (is_wp_error($download_file)) {
            ...
            if ($errorMsg == 'Forbidden') {
                $errorMsg = __('access is deined, this could be because your payment has expired.', 'wp-qore');
            } elseif (!$errorMsg) {
                $errorMsg = __('you have exceeded your download limit for this extension on this site.', 'wp-qore');
            }

            throw new Exception(__('There was an error downloading your premium extension because', 'wp-qore') . " $errorMsg");
        }
        ...
        if (is_wp_error($result)) {
            ...
            if (in_array($errorCode, array('copy_failed', 'incompatible_archive'))) {
                $errorMsg = sprintf(__("'%s' is not writeable.", 'wp-qore'), EXTENSIONS_DIR);
            }

            throw new Exception(__('There was an error installing your premium extension because', 'wp-qore') . " $errorMsg");
        }
        ...
        update_option('wpq2dbx-premium-extensions', $extensions);
    ...
    public function get_output()
    {
        foreach ($this->objectCache as $obj) {
            if ($obj && $obj->get_type() == WPQ2DBX_Extension_Base::TYPE_OUTPUT && $obj->is_enabled()) {
                return $obj;
            }
        }
    ...
    public function add_menu_items()
    {
        foreach ($this->objectCache as $obj) {
            ...
            add_submenu_page('b2dbx', $title, $title, 'activate_plugins', $slug, $func);
        }
    }
    ...
    {
        $class = 'WPQ2DBX_Extension_' . str_replace(' ', '', ucwords($name));
