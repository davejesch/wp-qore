...
class WPQ2DBX_Config
{
    ...
    public function set_option($name, $value)
    {
        ...
        $exists = $this->db->get_var(
            $this->db->prepare("SELECT * FROM {$this->db->prefix}wpq2dbx_options WHERE name = %s", $name)
        );

        if (is_null($exists)) {
            $this->db->insert($this->db->prefix . "wpq2dbx_options", array(
                'name' => $name,
                'value' => $value,
            ));
        } else {
            $this->db->update(
                $this->db->prefix . 'wpq2dbx_options',
                array('value' => $value),
                array('name' => $name)
            );
        }
    ...
    public function get_option($name, $no_cache = false)
    {
        if (!isset($this->options[$name]) || $no_cache) {
            $this->options[$name] = $this->db->get_var(
                $this->db->prepare("SELECT value FROM {$this->db->prefix}wpq2dbx_options WHERE name = %s", $name)
            );
        }
    ...
    public function complete()
    {
        ...
        $processed = new WPQ2DBX_Processed_Files();
        $processed->truncate();

        $processed = new WPQ2DBX_Processed_DBTables();
        $processed->truncate();
    ...
    {
        $in_progress = $this->db->get_var("SELECT value FROM {$this->db->prefix}wpq2dbx_options WHERE name = 'in_progress'");
        if (!$in_progress) {
            $msg = __('Backup stopped by user.', 'wp-qore');
