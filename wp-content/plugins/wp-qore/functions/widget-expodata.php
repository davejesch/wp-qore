...
if(current_user_can("manage_options"))
{
	...
	public static function init() {
		...
		add_action( 'wp_ajax_import_Widget_EXPOData', array( __CLASS__, 'ajax_import_Widget_EXPOData' ) );
	}
	...
	public static function add_admin_menus() {
		...
		add_action( 'admin_enqueue_scripts', function($hook) use ($export_page, $import_page){
			...
			wp_enqueue_style( 'Widget_EXPOData', plugins_url( 'css/widget-expodata.css', __FILE__ ) );
			wp_enqueue_script( 'Widget_EXPOData', plugins_url( 'js/widget-expodata.js', __FILE__ ), array( 'jquery', 'wp-ajax-response' ) );
			wp_localize_script( 'Widget_EXPOData', 'widgets_url', get_admin_url( false, 'widgets.php' ) );
		} );
	}
	...
	public static function export_settings_page() {
		...
		<div class="widget-expodata export-widget-settings">
			<div class="wrap">
				<h2><?php _e( 'Export Widgets', 'wp-qore' ); ?></h2>
				...
				<form action="" method="post" id="widget-export-settings">
					...
					<input class="button-bottom button-primary" type="submit" value="Export Widgets"/>
				</form>
			</div> <!-- end wrap -->
	...
	public static function import_settings_page() {
		...
		<div class="widget-expodata import-widget-settings">
			<div class="wrap">
				<h2><?php _e( 'Import Widgets', 'wp-qore' ); ?></h2>
				<?php if ( isset( $_FILES['widget-upload-file'] ) ) : ?>
					<div id="notifier" style="display: none;"></div>
					<div class="import-wrapper">
						<p>
							<a class="button select-all"><?php _e( 'check all', 'wp-qore' ); ?></a>
							<a class="button unselect-all"><?php _e( 'uncheck all', 'wp-qore' ); ?></a>
						</p>
						<form action="" id="import-widget-expodata" method="post">
							...
							<div class="title">
								<p class="widget-selection-error"><?php _e( 'Please select a widget to continue.', 'wp-qore' ); ?></p>
								<h3><?php _e( 'Sidebars', 'wp-qore' ); ?></h3>
								<div class="clear"></div>
							</div>
							<div class="sidebars">
								<?php
							...
							<input class="button-bottom button-primary" type="submit" name="import-widgets" id="import-widgets" value="<?php _e( 'Import Widget Settings', 'wp-qore'); ?>" />
						</form>
					</div>
				<?php else : ?>
					<form action="" id="upload-widget-expodata" method="post" enctype="multipart/form-data">
						<p><?php _e( 'Please select the widget export file to import settings.', 'wp-qore' ); ?></p>
						<p>
							<input type="text" disabled="disabled" class="file-name regular-text" />
							<a id="upload-button" class="button upload-button"><?php _e( 'Select a file', 'wp-qore' ); ?></a>
							<input type="file" name="widget-upload-file" id="widget-upload-file" size="40" style="display:none;" />
						</p>
						<input type="submit" name="button-upload-submit" id="button-upload-submit" class="button" value="<?php _e( 'Show Widget Settings', 'wp-qore' ); ?>" />
					</form>
				<?php endif; ?>
			</div> <!-- end wrap -->
	...
	public static function parse_import_data( $import_array ) {
		...
		$Widget_EXPOData = $import_array[1];
		...
		foreach ( $sidebars_data as $import_sidebar => $import_widgets ) :
			...
			foreach ( $import_widgets as $import_widget ) :
				...
				if ( isset( $current_sidebars[$import_sidebar] ) ) :
					...
					$current_Widget_EXPOData = get_option( 'widget_' . $title );
					...
					if ( array_key_exists( $title, $new_widgets ) ) {
						$new_widgets[$title][$new_index] = $Widget_EXPOData[$title][$index];
						$multiwidget = $new_widgets[$title]['_multiwidget'];
						unset( $new_widgets[$title]['_multiwidget'] );
						$new_widgets[$title]['_multiwidget'] = $multiwidget;
					} else {
						$current_Widget_EXPOData[$new_index] = $Widget_EXPOData[$title][$index];
						$current_multiwidget = $current_Widget_EXPOData['_multiwidget'];
						$new_multiwidget = isset($Widget_EXPOData[$title]['_multiwidget']) ? $Widget_EXPOData[$title]['_multiwidget'] : false;
						$multiwidget = ($current_multiwidget != $new_multiwidget) ? $current_multiwidget : 1;
						unset( $current_Widget_EXPOData['_multiwidget'] );
						$current_Widget_EXPOData['_multiwidget'] = $multiwidget;
						$new_widgets[$title] = $current_Widget_EXPOData;
					}
	...
	public static function export_widget_settings() {
		...
		if ( isset( $_POST['action'] ) && $_POST['action'] == 'export_widget_settings' ){
			...
			header( "Content-Disposition: attachment; filename=Widget_EXPOData.json" );
	 ...
	 */
	public static function ajax_import_Widget_EXPOData() {
		$response = array(
		...
		foreach ( $widgets as $widget_title => $widget_value ) {
			foreach ( $widget_value as $widget_key => $widget_value ) {
				$widgets[$widget_title][$widget_key] = $Widget_EXPOData[$widget_title][$widget_key];
			}
		}

		$sidebar_data = array( array_filter( $sidebar_data ), $widgets );
		$response['id'] = ( self::parse_import_data( $sidebar_data ) ) ? true : new WP_Error( 'widget_import_submit', __( 'Unknown Error', 'wp-qore' ) );
