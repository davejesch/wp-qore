...
class WPQ2DBX_DatabaseBackup
{
    ...
    {
        $this->database = WPQ2DBX_Factory::db();
        $this->config = WPQ2DBX_Factory::get('config');
        $this->processed = $processed ? $processed : new WPQ2DBX_Processed_DBTables();
    ...
    public function execute()
    {
        ...
        foreach ($tables as $t) {
            ...
            if (!$this->processed->is_complete($tableName)) {
                ...
                if ($count > 0) {
                    WPQ2DBX_Factory::get('logger')->log(sprintf(__("Resuming table '%s' at row %s.", 'wp-qore'), $tableName, $count));
                }

                $this->backup_database_table($tableName, $count);

                WPQ2DBX_Factory::get('logger')->log(sprintf(__("Processed table '%s'.", 'wp-qore'), $tableName));
            }
        }
    }
    ...
    public function get_file()
    {
        ...
        return $file . '.' . WPQ2DBX_Factory::secret(DB_NAME);
    }
    ...
    private function write_db_dump_header()
    {
        ...
        if (!is_writable($dump_location)) {
            $msg = sprintf(__("A database backup cannot be created because WordPress does not have write access to '%s', please ensure this directory has write access.", 'wp-qore'), $dump_location);
            WPQ2DBX_Factory::get('logger')->log($msg);
        ...
        $this->write_to_temp("-- Version " . WPQORE_2_DROPBOX_VERSION . "\n");
        $this->write_to_temp("-- http://wpq2dbx.com\n");
    ...
    {
        $db_error = __('Error while accessing database.', 'wp-qore');
    ...
    private function write_to_temp($out)
    {
        ...
        if (fwrite($this->temp, $out) === false) {
            throw new Exception(__('Error writing to php://memory.', 'wp-qore'));
        }
    }
    ...
    private function persist()
    {
        ...
        if (!fclose($fh)) {
            throw new Exception(__('Error closing sql dump file.', 'wp-qore'));
        }

        if (!fclose($this->temp)) {
            throw new Exception(__('Error closing php://memory.', 'wp-qore'));
        }
